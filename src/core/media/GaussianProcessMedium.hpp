...
protected:
    GPCorrelationContext _ctxt = GPCorrelationContext::RenewalPlus;
    ...
    const bool _dumpDataVisCovOnly = false;
...
public:
    ...
    GaussianProcessMedium(std::shared_ptr<GPSampleNode> gp, std::vector<std::shared_ptr<PhaseFunction>> phases,
        ...
        GPCorrelationContext ctxt = GPCorrelationContext::RenewalPlus, GPIntersectMethod intersectMethod = GPIntersectMethod::GPDiscrete, GPNormalSamplingMethod normalSamplingMethod = GPNormalSamplingMethod::ConditionedGaussian) :
    ...
    virtual bool sampleGradient(PathSampleGenerator& sampler, const Ray& ray, const Vec3d& isect_p, const float& isect_t,
        MediumState& state, Vec3d& grad) const = 0;
    ...
    bool intersect(PathSampleGenerator& sampler, const Ray& ray, MediumState& state, double& t, bool firstIsectAlongRay) const;
    virtual bool intersectGP(PathSampleGenerator& sampler, const Ray& ray, MediumState& state, double& t, bool firstIsectAlongRay) const = 0;
    ...
    virtual Vec3f transmittance(PathSampleGenerator &sampler, const Ray &ray, bool startOnSurface, bool endOnSurface, MediumState* state) const override;
