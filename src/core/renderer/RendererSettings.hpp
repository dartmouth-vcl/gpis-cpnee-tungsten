...
class RendererSettings : public JsonSerializable
{
    ...
    Vec2u _dataDumpPixel;
    std::string _dataDumpPath;
...
public:
    ...
    void setDataDumpStats(const std::string pixel_str)
    {
        size_t comma = pixel_str.find_last_of(',');
        uint pixel_x = std::atoi(pixel_str.substr(0, comma).c_str());
        uint pixel_y = std::atoi(pixel_str.substr(comma+1, pixel_str.size()).c_str());
        _dataDumpPixel = Vec2u(pixel_x, pixel_y);
        Path dataDumpPath = _hdrOutputFile;
        dataDumpPath = dataDumpPath.stripExtension() + "-" + pixel_str + ".json";
        _dataDumpPath = dataDumpPath.asString();
    }

    Vec2u dataDumpPixel() const {
        return _dataDumpPixel;
    }

    std::string dataDumpPath() const {
        return _dataDumpPath;
    }
    ...
    void setHdrOutputFile(const Path &file)
    {
        _hdrOutputFile = file;
        for (auto &b : _outputs) {
            Path bufferFilePath = file.parent() + b.typeString() + "/" + file.baseName() + "-" + b.typeString() + file.extension();
            b.setHdrOutputFile(bufferFilePath);
        }
    }

    std::string logFilePath() const {
        Path logFilePath = _hdrOutputFile;
        logFilePath = logFilePath.parent() + "log/" + logFilePath.baseName() + ".log";
        return logFilePath.asString();
    }
