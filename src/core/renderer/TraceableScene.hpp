...
private:
    ...
    std::unique_ptr<std::ofstream> _dataDumpFile;
...
public:
    ...
    : _cam(cam),
      ...
      _seed(seed)
    {
        std::string path = _settings.dataDumpPath();
        if (path != "") {
            std::cout << "Write data dump to: " << path << std::endl;
            _dataDumpFile = std::make_unique<std::ofstream>(path);
        }
    ...
    ~TraceableScene()
    {
        if (_dataDumpFile && _dataDumpFile->is_open())
            _dataDumpFile->close();
    ...
    std::ofstream& dataDumpFile() const {
        if (!_dataDumpFile) {
            throw std::runtime_error("Data dump file is not initialized");
        }
        return *_dataDumpFile;
    }
};
