...
Vec3f BidirectionalPathTracer::traceSample(Vec2u pixel, uint32 lightPathId, PathSampleGenerator &sampler, uint spp)
{
    ...
    Vec4u pixelSampleSegment = Vec4u(pixel.x(), pixel.y(), spp, 0u);
    cameraPath.tracePath(*_scene, *this, sampler, pixelSampleSegment, true);
    sampler.startPath(0, lightPathId);
    pixelSampleSegment.w() = cameraPath.maxLength() - 1;
    emitterPath.tracePath(*_scene, *this, sampler, pixelSampleSegment, false);
