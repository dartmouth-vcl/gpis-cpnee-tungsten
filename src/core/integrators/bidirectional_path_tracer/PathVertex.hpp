...
class PathVertex
{
    ...
    std::shared_ptr<Medium::MediumState> _mediumState;
...
public:
    ...
    bool sampleNextVertex(const TraceableScene &scene, TraceBase &tracer, TraceState &state, bool adjoint,
                          bool fromCamera, PathVertex *prev, PathEdge *prevEdge, PathVertex &next, PathEdge &nextEdge);
    ...
    Medium::MediumState *mediumState(bool fromCamera) const
    {
        Medium::MediumState *mediumState = _mediumState.get();
        mediumState->info.pixelSampleSegment.w() += fromCamera ? 1 : -1;
        return mediumState;
    }
