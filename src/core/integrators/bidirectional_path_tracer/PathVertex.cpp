...
bool PathVertex::sampleRootVertex(TraceState &state)
{
    ...
    case EmitterVertex: {
        ...
        _mediumState = std::make_shared<Medium::MediumState>(state.mediumState);
        return true;
    } case CameraVertex: {
...
bool PathVertex::sampleNextVertex(const TraceableScene &scene, TraceBase &tracer, TraceState &state, bool adjoint, bool fromCamera, PathVertex *prev, PathEdge */*prevEdge*/, PathVertex &next, PathEdge &nextEdge)
{
    Vec3f weight;
    float pdf;
    ...
    state.mediumState.info.pixelSampleSegment.w() += fromCamera ? 1 : -1;
    ...
    if (!hitSurface) {
        ...
        next._mediumState = std::make_shared<Medium::MediumState>(state.mediumState);
        state.bounce++;
