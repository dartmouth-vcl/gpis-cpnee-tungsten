...
void LightPath::tracePath(const TraceableScene &scene, TraceBase &tracer, PathSampleGenerator &sampler, Vec4u pixelSampleSegment, bool fromCamera, int length, bool prunePath)
{
    if (length == -1)
        length = _maxLength;
    ...
    state.mediumState.info.pixelSampleSegment = pixelSampleSegment;
    state.mediumState.info.t = 0;
    state.mediumState.reset();
    ...
    while (_length < length) {
        if (!_vertices[_length - 1].sampleNextVertex(scene, tracer, state, _adjoint, fromCamera,
        ...
        int s, int t, int maxBounce, PathSampleGenerator &sampler, float *ratios)
{
    ...
    if (s == 1 && a.emitter()->isInfinite()) {
        ...
        Vec3f transmittance = tracer.generalizedShadowRayAndPdfs(sampler, ray, b.selectMedium(-d), nullptr,
                bounce, b.onSurface(), true, b.mediumState(true), edge.pdfBackward, edge.pdfForward);
        ...
        Vec3f transmittance = tracer.generalizedShadowRayAndPdfs(sampler, ray, a.selectMedium(edge.d), nullptr,
                bounce, a.onSurface(), b.onSurface(), a.mediumState(false), edge.pdfForward, edge.pdfBackward);
        if (transmittance == 0.0f)
            return Vec3f(0.0f);
        ...
        int s, int maxBounce, PathSampleGenerator &sampler, Vec3f &weight, Vec2f &pixel, float *ratios)
{
    ...
    PathEdge edge(a, b);
    Ray ray(a.pos(), edge.d, 1e-4f, edge.r*(1.0f - 1e-4f));
    Vec3f transmittance = tracer.generalizedShadowRayAndPdfs(sampler, ray, a.selectMedium(edge.d), nullptr, bounce, a.onSurface(), b.onSurface(), a.mediumState(false), edge.pdfForward, edge.pdfBackward);

    // TODO: GP random seed for light path does not align with camera path (due to varying length) -> alternative way of seeding?
    // std::cout << a.mediumState(false)->info.pixelSampleSegment.w() << std::endl; // Debug GP random seed for path
