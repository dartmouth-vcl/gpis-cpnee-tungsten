...
struct WeightSpaceRealization {
    ...
    double evaluate(const Vec3d& p, int& GPId) const;
    ...
    static WeightSpaceRealization sample(std::shared_ptr<WeightSpaceBasis> basis, std::shared_ptr<GaussianProcess> gp, const Vec4u pixelSampleSegment, const uint globalSeed);
};
...
struct WeightSpaceBasis {
    ...
    static WeightSpaceBasis sample(std::shared_ptr<CovarianceFunction> cov, int n, const Vec4u pixelSampleSegment, Vec3d spectralLoc = Vec3d(0.), bool sort = false, int d = 2);
