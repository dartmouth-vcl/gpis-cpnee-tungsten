...
namespace Tungsten {
    ...
    struct GPRealNodeValues : GPRealNode {
        GPRealNodeValues(const Eigen::MatrixXd& values, const Eigen::VectorXi& ids, const GaussianProcess* gp) : _values(values), _ids(ids), _gp(gp) {}
    ...
    public:
        ...
        virtual std::shared_ptr<GaussianProcess> get_gaussian_process() = 0;
        virtual const GaussianProcess* get_gaussian_process_from_id(int id) = 0;
    };
    ...
    private:
        ...
        std::shared_ptr<GaussianProcess> _csg;
    ...
    public:
        ...
        std::shared_ptr<GaussianProcess> get_gaussian_process() override {
            return _csg;
        }

        const GaussianProcess* get_gaussian_process_from_id(int id) override;
    };

    class GaussianProcess : public GPSampleNode, public std::enable_shared_from_this<GaussianProcess> {
    public:
        ...
        // Transform the posOrDir at the local position at p. The local position is used for constructing the ONB from the mean grad at p. 
        virtual Vec3f transformPosDirWorldtoLocal(const Vec3f& p, const Vec3f& posOrDir, const float localScale) const;
        virtual Vec3f transformPosDirLocaltoWorld(const Vec3f& p, const Vec3f& posOrDir, const float localScale) const;
        virtual Vec3f transformGradWorldtoLocal(const Vec3f& p, const Vec3f& grad, const float localScale) const;
        virtual Vec3f transformGradLocaltoWorld(const Vec3f& p, const Vec3f& grad, const float localScale) const;
        ...
        std::tuple<Eigen::VectorXd, Eigen::VectorXi, CovMatrix> mean_and_cov(
            const Vec3d* points, const Derivative* derivative_types, const Vec3d* ddirs,
            Vec3d deriv_dir, size_t numPts) const;

        std::tuple<Eigen::VectorXd, Eigen::VectorXi> mean_with_id(
                const Vec3d* points, const Derivative* derivative_types, const Vec3d* ddirs,
                Vec3d deriv_dir, size_t numPts) const;
        ...
        std::tuple<Eigen::VectorXd, Eigen::VectorXi> mean_prior(
            const Vec3d* points, const Derivative* derivative_types, const Vec3d* ddirs,
            Vec3d deriv_dir, size_t numPts) const;

        Eigen::Vector2d mean_weight_space(
        ...
        std::tuple<MultivariateNormalDistribution, Eigen::VectorXi> create_mvn_cond(
            const Vec3d* points, const Derivative* derivative_types, size_t numPts,
            const Vec3d* ddirs,
