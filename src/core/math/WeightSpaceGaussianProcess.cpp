...
double WeightSpaceRealization::evaluate(const Vec3d& p, int& GPId) const {
    Derivative d = Derivative::None;
    double c = (*gp->_cov)(Derivative::None, Derivative::None, p, p, Vec3d(), Vec3d());
    double scale = sqrt(c);
    Eigen::Vector2d mean_and_id = gp->mean_weight_space(p, d);
    GPId = (int)mean_and_id(1);
    return scale * basis->evaluate(vec_conv<Eigen::Vector3d>(gp->shell_embedding(p)), weights) + mean_and_id(0);
}
...
Vec3d WeightSpaceRealization::evaluateGradient(const Vec3d& p) const {
    ...
    Eigen::Vector2d mean_and_id = gp->mean_weight_space(p, d);
    Vec3d meanGrad = mean_and_id(1) == gp->_id ? gp->_mean->dmean_da(p) : gp->_mean_additional->dmean_da(p);
    
    return basisGrad + meanGrad;
}
...
Eigen::VectorXd WeightSpaceRealization::evaluate(const Vec3d* ps, size_t num_ps) const {
    ...
    for (size_t p = 0; p < num_ps; p++) {
        int GPIdTmp;
        result[p] = evaluate(ps[p], GPIdTmp);
    }
    return result;
}
...
WeightSpaceBasis WeightSpaceBasis::sample(std::shared_ptr<CovarianceFunction> cov, int n, const Vec4u pixelSampleSegment, Vec3d spectralLoc, bool sort, int d) {
    auto sampler = reinterpret_cast<PathSampleGenerator*>(new UniformPathSampler(MathUtil::xxhash32(pixelSampleSegment)));
...
WeightSpaceRealization WeightSpaceRealization::sample(std::shared_ptr<WeightSpaceBasis> basis, std::shared_ptr<GaussianProcess> gp, const Vec4u pixelSampleSegment, const uint globalSeed) {
    auto sampler = reinterpret_cast<PathSampleGenerator*>(new UniformPathSampler(globalSeed + MathUtil::xxhash32(pixelSampleSegment)));
    return WeightSpaceRealization{
