...
void GPSampleNodeCSG::fromJson(JsonPtr value, const Scene& scene) {
    ...
    auto left = std::static_pointer_cast<GaussianProcess>(_left);
    auto right = std::static_pointer_cast<GaussianProcess>(_right);
    _csg = std::make_shared<GaussianProcess>(*left);
    _csg->_mean_additional = right->_mean;
    _csg->_id = left->_id;
    _csg->_id_additional = right->_id;
}


const GaussianProcess* GPSampleNodeCSG::get_gaussian_process_from_id(int id) {
    auto left = std::static_pointer_cast<GaussianProcess>(_left);
    auto right = std::static_pointer_cast<GaussianProcess>(_right);
    if (id == left->_id)
        return left->get_gaussian_process_from_id(id);
    else
        return right->get_gaussian_process_from_id(id);
}
...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXi> GPRealNodeValues::flatten() const {
    Eigen::MatrixXi ids(_values.rows(), _values.cols());
...
void GaussianProcess::setConditioning(
    ...
    auto [mean, _] = mean_prior(globalCondPs.data(), globalCondDerivs.data(), globalCondDerivDirs.data(), Vec3d(0.), globalCondPs.size());
    _globalCondPriorMean = cond_values_view - mean;
...
std::tuple<Eigen::VectorXd, Eigen::VectorXi, CovMatrix> GaussianProcess::mean_and_cov(
    ...
    for (size_t i = 0; i < numPts; i++) {
        ...
        ps_id(i) = _id;
        if (_mean_additional) {
            auto ps_mean_add = (*_mean_additional)(derivative_types[i], points[i], ddir_a);
            if (ps_mean_add < ps_mean(i)) {
                ps_mean(i) = ps_mean_add;
                ps_id(i) = _id_additional;
            }
        }
    ...
    return { ps_mean, ps_id, ps_cov };
}

Eigen::Vector2d GaussianProcess::mean_weight_space(
        const Vec3d point, const Derivative derivative_type) const {

    Eigen::Vector2d ps_mean;
    ps_mean(0) = (*_mean)(derivative_type, point, Vec3d(0.));
    ps_mean(1) = _id;
    if (_mean_additional) {
        auto ps_mean_add = (*_mean_additional)(derivative_type, point, Vec3d(0.));
        if (ps_mean_add < ps_mean(0)) {
            ps_mean(0) = ps_mean_add;
            ps_mean(1) = _id_additional;
        }
    }
    return ps_mean;
}

std::tuple<Eigen::VectorXd, Eigen::VectorXi> GaussianProcess::mean_prior(
    const Vec3d * points, const Derivative * derivative_types, const Vec3d * ddirs,
    Vec3d deriv_dir, size_t numPts) const {
    ...
    for (size_t i = 0; i < numPts; i++) {
        ...
        ps_id(i) = _id;
        if (_mean_additional) {
            auto ps_mean_add = (*_mean_additional)(derivative_types[i], points[i], ddir);
            if (ps_mean_add < ps_mean(i)) {
                ps_mean(i) = ps_mean_add;
                ps_id(i) = _id_additional;
            }
    ...
    return { ps_mean, ps_id };
}

std::tuple<Eigen::VectorXd, Eigen::VectorXi> GaussianProcess::mean_with_id(
        const Vec3d* points, const Derivative* derivative_types, const Vec3d* ddirs,
        Vec3d deriv_dir, size_t numPts) const {

    auto [ps_mean, ps_id] = mean_prior(points, derivative_types, ddirs, deriv_dir, numPts);
    ...
    if (_globalCondPs.size() == 0) {
        ...
        return { ps_mean + (solved * _globalCondPriorMean), ps_id};
    }
}
...
Eigen::VectorXd GaussianProcess::mean(
    ...
    auto [ps_mean, _] = mean_prior(points, derivative_types, ddirs, deriv_dir, numPts);
...
std::shared_ptr<GPRealNode> GaussianProcess::sample_start_value(Vec3d p, PathSampleGenerator& sampler) const {
    ...
    auto [mean, id, cov] = mean_and_cov(&p, &deriv, nullptr, Vec3d(0.), 1);
    ...
    return std::make_shared<GPRealNodeValues>(vals, id, this);
}
...
std::shared_ptr<GPRealNode> GaussianProcess::sample(
    ...
    auto [ps_mean, ps_id, ps_cov] = mean_and_cov(points, derivative_types, deriv_dirs, deriv_dir, numPts);
    auto mvn = MultivariateNormalDistribution(ps_mean, ps_cov);
    return std::make_shared<GPRealNodeValues>(mvn.sample(constraints, numConstraints, samples, sampler), ps_id, this);
}
...
std::shared_ptr<GPRealNode> GaussianProcess::sample_cond(
    ...
    auto [mvn, id] = create_mvn_cond(points, derivative_types, numPts, deriv_dirs,
        cond_points, real->_values.data(), cond_derivatives, numCondPts, cond_deriv_dirs,
        deriv_dir);

    return std::make_shared<GPRealNodeValues>(mvn.sample(constraints, numConstraints, samples, sampler), id, this);
}
...
double GaussianProcess::eval(
    ...
    auto [ps_mean, _, ps_cov] = mean_and_cov(points, derivative_types, ddirs, deriv_dir, numPts);
    auto mvn = MultivariateNormalDistribution(ps_mean, ps_cov);
...
std::tuple<MultivariateNormalDistribution, Eigen::VectorXi> GaussianProcess::create_mvn_cond(
    const Vec3d* points, const Derivative* derivative_types, size_t numPts,
    const Vec3d* ddirs,
    ...
    Vec3d deriv_dir) const {
    // Note (for myself): ddirs = nullptr, cond_ddirs = nullptr, deriv_dir = ray direction

    if (numCondPts == 0) {
        auto [ps_mean, ps_id, ps_cov] = mean_and_cov(points, derivative_types, ddirs, deriv_dir, numPts);
        return { MultivariateNormalDistribution(ps_mean, ps_cov), ps_id };
    }
    ...
    CovMatrix solved;
    // Note: when playing around with the function space implementation,
    // pseudo inverse is the only solver that gives valid result for conditioning
    ...
    auto [m2, id] = mean_with_id(points, derivative_types, ddirs, deriv_dir, numPts);
    m2 += solved * (cond_values_view - mean(cond_points, cond_derivative_types, cond_ddirs, deriv_dir, numCondPts));
    ...
    return { MultivariateNormalDistribution(m2, s2), id };
}
...
double GaussianProcess::eval_cond(
    ...
    Eigen::Map<const Eigen::VectorXd> eval_values_View(values, numPts);
    auto [mvn, _] = create_mvn_cond(points, derivative_types, numPts, ddirs,
...
double GaussianProcess::goodStepsize(Vec3d p, double targetCov, Vec3d rd) const
{
    ...
    double cov = (*_cov)(Derivative::None, Derivative::None, shell_embedding(p), shell_embedding(p + rd * stepsize_avg), Vec3d(0.), Vec3d(0.)) / sigma;
...
Vec3f GaussianProcess::transformPosDirWorldtoLocal(const Vec3f& p, const Vec3f& posOrDir, const float localScale) const{
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        // local wrt normal.
        TangentFrame frame(mean_grad);
        Vec3f posOrDirLocal = frame.toLocal(posOrDir);
        Vec3f posOrDirLocalIso = this->_cov->transformPosDirWorldtoLocal(posOrDirLocal, localScale);
        return frame.toGlobal(posOrDirLocalIso);
    }
    else{
        return this->_cov->transformPosDirWorldtoLocal(posOrDir, localScale);
    }

}

Vec3f GaussianProcess::transformPosDirLocaltoWorld(const Vec3f& p, const Vec3f& posOrDir, const float localScale) const{
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        // local wrt normal. 
        TangentFrame frame(mean_grad);
        Vec3f posOrDirLocalIso = frame.toLocal(posOrDir);
        Vec3f posOrDirLocal = this->_cov->transformPosDirLocaltoWorld(posOrDirLocalIso, localScale);
        return frame.toGlobal(posOrDirLocal);
    }
    else{
        return this->_cov->transformPosDirLocaltoWorld(posOrDir, localScale);
    }
}

Vec3f GaussianProcess::transformGradWorldtoLocal(const Vec3f& p, const Vec3f& grad, const float localScale) const{
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        TangentFrame frame(mean_grad);
        Vec3f gradLocal = frame.toLocal(grad);
        Vec3f gradLocalIso = this->_cov->transformGradWorldtoLocal(gradLocal, localScale);
        return frame.toGlobal(gradLocalIso);    
    }
    else{
        return this->_cov->transformGradWorldtoLocal(grad, localScale);
    }
    
}

Vec3f GaussianProcess::transformGradLocaltoWorld(const Vec3f& p, const Vec3f& grad, const float localScale) const{
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        TangentFrame frame(mean_grad);
        Vec3f gradLocalIso = frame.toLocal(grad);
        Vec3f gradLocal = this->_cov->transformGradLocaltoWorld(gradLocalIso, localScale);
        return frame.toGlobal(gradLocal);
    }
    else{
        return this->_cov->transformGradLocaltoWorld(grad, localScale);
    }
}

Eigen::Matrix3f GaussianProcess::localToWorldInvTransposeMatrix(const Vec3f& p) const{
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        TangentFrame frame(mean_grad);
        Eigen::Matrix3f T(3, 3);
        T.col(0) << frame.tangent.x(), frame.tangent.y(), frame.tangent.z();
        T.col(1) << frame.bitangent.x(), frame.bitangent.y(), frame.bitangent.z();
        T.col(2) << frame.normal.x(), frame.normal.y(), frame.normal.z();
        return T * this->_cov->localToWorldInvTransposeMatrix(p) * T.transpose();
    }
    else{
        return this->_cov->localToWorldInvTransposeMatrix(p);
    }
    
}

Eigen::Matrix3f GaussianProcess::worldToLocalInvTransposeMatrix(const Vec3f& p) const {
    if(this->_cov->_localAniso){
        Eigen::Vector2d mean_and_id = this->mean_weight_space(Vec3d(p), Derivative::None);
        Vec3f mean_grad = Vec3f(mean_and_id(1) == this->_id ? this->_mean->dmean_da(Vec3d(p)) : this->_mean_additional->dmean_da(Vec3d(p)));
        TangentFrame frame(mean_grad);
        ...
        T.col(1) << frame.bitangent.x(), frame.bitangent.y(), frame.bitangent.z();
        T.col(2) << frame.normal.x(), frame.normal.y(), frame.normal.z();
        return T * this->_cov->worldToLocalInvTransposeMatrix(p) * T.transpose();
    }
    else{
        return this->_cov->worldToLocalInvTransposeMatrix(p);
    }
}
