...
public:
    ...
    virtual bool sampleDirect1D(uint32 threadIndex, const Vec3f &p, PathSampleGenerator &sampler, 
        LightSample &sample, const IntersectionInfo& info,
        const CircleSegment& segment, const Vec3f& gu, const Vec3f& g_fix, 
        const float std_gu, const SegmentSamplingMethod samplingMethod = SegmentSamplingMethod::Gaussian) const;
    virtual DirectionCircle getLightCircle(const Vec3f &p) const;
    ...
    virtual float directPdf1D(uint32 threadIndex, const IntersectionTemporary &data,
            const IntersectionInfo &info, const Vec3f &p, 
            const CircleSegment& segment, const Vec3f& gu, const Vec3f& g_fix,
            const float std_gu, const SegmentSamplingMethod samplingMethod = SegmentSamplingMethod::Gaussian) const;
    ...
    virtual BSphere3f sbounds() const;
