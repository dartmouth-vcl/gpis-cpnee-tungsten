...
bool Sphere::samplePosition(PathSampleGenerator &sampler, PositionSample &sample) const
{
    ...
    if (sample.uv.x() > 1.0f)
        sample.uv.x() -= 1.0f;
    sample.weight = PI*_area*_emScale * (*_emission)[sample.uv];
...
{
    return PI*_emScale * (*_emission)[sample.uv];
}
...
{
    return data.as<SphereIntersection>()->backSide ? Vec3f(0.0f) : _emScale * (*_emission)[info.uv];
}
...
float Sphere::approximateRadiance(uint32 /*threadIndex*/, const Vec3f &p) const
{
    if (!isEmissive())
        return 0.0f;
    return solidAngle(p)*_emScale*_emission->average().max();
}
...
BSphere3f Sphere::sbounds() const
{
    return BSphere3f(_pos, _radius + 1.0e-6f);
}
