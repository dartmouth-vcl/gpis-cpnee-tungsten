...
bool InfiniteSphere::sampleDirection(PathSampleGenerator &/*sampler*/, const PositionSample &point, DirectionSample &sample) const
{
    ...
    sample.weight = _emScale * (*_emission)[point.uv]/sample.pdf;
...
BSphere3f InfiniteSphere::sbounds() const
{
    return BSphere3f(Vec3f(0.0f), 1e30f);
}
