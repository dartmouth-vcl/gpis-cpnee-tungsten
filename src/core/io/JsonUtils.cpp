...
rapidjson::Value toJson(const Eigen::Matrix3f &value, rapidjson::Document::AllocatorType &allocator)
{
    rapidjson::Value array(rapidjson::kArrayType);
    array.Reserve(9, allocator);

    // Store in row-major order
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 3; ++col) {
            array.PushBack(rapidjson::Value(prettifyFloatToDouble(value(row, col))), allocator);
        }
    }
