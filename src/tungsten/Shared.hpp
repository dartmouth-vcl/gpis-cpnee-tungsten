...
static const int OPT_DATA_DUMP_PIXEL   = 12;
...
public:
      ...
      _threadCount(max(ThreadUtils::idealThreadCount() - 1, 1u))
    {
        ...
        parser.addOption('p', "data-dump-pixel", "Specifies the pixel which to dump data for the primary ray", true, OPT_DATA_DUMP_PIXEL);
    }
    ...
    bool renderScene()
    {
        ...
        if (_parser.isPresent(OPT_DATA_DUMP_PIXEL)) {
            const std::string pixel_str = _parser.param(OPT_DATA_DUMP_PIXEL);
            _scene->rendererSettings().setDataDumpStats(pixel_str);
        }
        ...
        std::string logFilePath = _scene->rendererSettings().logFilePath();
        _logStreamFile = std::ofstream(logFilePath);
        ...
        try {
            ...
            while (!integrator.done()) {
                ...
                float time_total_est = time_elapsed * (float)maxSpp / (float)integrator.currentSpp();
                float time_remaining_est = time_total_est - time_elapsed;
                writeLogLine(tfm::format("Completed %d/%d spp, %.2f/%.2f sec, remaining %.2f sec", integrator.currentSpp(), maxSpp, time_elapsed, time_total_est, time_remaining_est));
